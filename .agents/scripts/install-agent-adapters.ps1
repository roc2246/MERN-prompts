...
Copy-Item `
    -Path (Join-Path $AgentsRoot 'adapters/github/copilot-instructions.md') `
    -Destination (Join-Path $GitHubDir 'copilot-instructions.md') `
    -Force

# Claude Code expects CLAUDE.md at the repository root.
Copy-Item `
    -Path (Join-Path $AgentsRoot 'adapters/CLAUDE.md') `
    -Destination (Join-Path $ProjectRoot 'CLAUDE.md') `
    -Force
