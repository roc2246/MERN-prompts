...
If the client supports Agent Skills, use `.agents/skills/mern-test-coverage/SKILL.md` instead of treating this file as a large standalone system prompt. The skill is the source of truth for this workflow.

## Task

Audit the requested source scope for missing unit tests, propose exact tests, and generate them only after approval using the existing test framework.
...
- Load `.agents/skills/mern-test-coverage/references/project-standards.md` only when repository conventions affect the task.
- For a deep or exhaustive audit, load `.agents/skills/mern-test-coverage/references/detailed-checklist.md`. Do not load the detailed checklist for a narrow question.
