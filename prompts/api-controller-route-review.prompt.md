...
If the client supports Agent Skills, use `.agents/skills/mern-api-review/SKILL.md` instead of treating this file as a large standalone system prompt. The skill is the source of truth for this workflow.

## Task

Review the requested Express routes/controllers/middleware for API correctness, validation, auth/authz, status codes, error handling, and separation of concerns.
...
- Load `.agents/skills/mern-api-review/references/project-standards.md` only when repository conventions affect the task.
- For a deep or exhaustive audit, load `.agents/skills/mern-api-review/references/detailed-checklist.md`. Do not load the detailed checklist for a narrow question.
