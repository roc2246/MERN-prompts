...
If the client supports Agent Skills, use `.agents/skills/mern-scss-review/SKILL.md` instead of treating this file as a large standalone system prompt. The skill is the source of truth for this workflow.

## Task

Audit the requested SCSS/CSS scope for architecture, nesting, duplication, variables/mixins, responsiveness, and maintainability while preserving appearance.
...
- Load `.agents/skills/mern-scss-review/references/project-standards.md` only when repository conventions affect the task.
- For a deep or exhaustive audit, load `.agents/skills/mern-scss-review/references/detailed-checklist.md`. Do not load the detailed checklist for a narrow question.
